- [LangChain(1) - 사용하는 이유, RAG 워크플로우](#langchain1---사용하는-이유-rag-워크플로우)
...
# LangChain(1) - 사용하는 이유, RAG 워크플로우

<br>

## LangChain 이란?

LangChain은<<mark>>**RAG나 Agent를 쉽게**</</mark>> 만들 수 있게 도와주는 오픈소스 프레임워크이다.
...
검색기, 벡터DB, 파일 로더, API 같은<<mark>>**여러 도구를**</</mark>>
...
이것들을 모두 같은 코드로<<mark>>**추상화해서 통합해 제공**</</mark>>한다.

<br>

그래서

- 모델을 바꾸더라도<<mark>>**코드 구조는 거의 그대로 유지**</</mark>>
- 새로운 도구가 나와도<<mark>>**쉽게 교체하거나 추가 가능**</</mark>>

이런 장점은 요즘 굉장히 빠르게 발전해<<mark>>**변화하는 AI 생태계에 적응**</</mark>>할 수 있게한다.
...
이 패키지들는 랭체인 본체에 다 들어 있지 않고,<<mark>>**회사나 기능별로 분리된 패키지를 설치**</</mark>>해야한다.
...
1.<<mark>>**Input processing**</</mark>> – 원시 데이터를 구조화된 문서로 바꾸는 단계 2.<<mark>>**Embedding & storage**</</mark>> – 텍스트를 벡터로 바꿔 저장하는 단계 3.<<mark>>**Retrieval**</</mark>> – 사용자 질문과 관련된 정보를 찾는 단계 4.<<mark>>**Generation**</</mark>> – AI가 답변을 생성하는 단계
